    ...
    return (
      ...
      >
        <div className="rce-popup">
          {props.renderHeader ? (
            <div className="rce-popup-header">{props.renderHeader()}</div>
          ) : (
            <div className="rce-popup-header">
              <span>{props.header}</span>
              {props.header &&
                props.headerButtons.map((x, i) => <Button key={i} {...x} />)}
            </div>
          )}
          <div className="rce-popup-content" style={{ color: props.color }}>
            {props.renderContent ? props.renderContent() : props.text}
          </div>
          <div className="rce-popup-footer">
            {props.renderFooter
              ? props.renderFooter()
              : props.footerButtons.map((x, i) => <Button key={i} {...x} />)}
          </div>
        </div>
      </div>
    );
